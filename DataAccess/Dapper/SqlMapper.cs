...
namespace _Dapper {
  ...
  public static partial class SqlMapper {
    ...
    // added by kevin
    // checks to see if the "sql" begins with a SQL keyword, otherwise it is a SP
    public static CommandType GetCommandTypeFromSQL(string sql) {
      CommandType cmdType = CommandType.StoredProcedure;

      // replace all white space (return, newline, tabs, multi space... ) with a single space.
      sql = Regex.Replace(sql, @"\s+", " ");

      sql = sql.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)[0].ToUpper();
      if("..SELECT.UPDATE.DELETE.INSERT.WITH.DECLARE./*SQL*/./**".IndexOf(sql) > 0) {
    ...
    public static IEnumerable<T> Query<T>(
      ...
      if(commandType == null) {
        commandType = GetCommandTypeFromSQL(sql);
      }
    ...
    private static IDbCommand SetupCommand(IDbConnection cnn, IDbTransaction transaction, string sql, Action<IDbCommand, object> paramReader, object obj, int? commandTimeout, CommandType? commandType) {
      ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      // added by kevin checking for sql keywords to determine if SP or TextStatement
      if(commandType == null) {
        commandType = GetCommandTypeFromSQL(sql);
      }
      // end --- added by kevin
